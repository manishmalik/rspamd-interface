...
var Chacha20 = function(key, nonce, counter) {
  ...
  if(nonce.length == 24)
  {
    this.input[12] = U8TO32_LE(nonce, 0);
    this.input[13] = U8TO32_LE(nonce, 4);
    this.input[14] = U8TO32_LE(nonce, 8);
    this.input[15] = U8TO32_LE(nonce, 12);
...
nacl.secretbox = function(msg, nonce, key) {
  ...
  var c = new Uint8Array(msg.length + crypto_secretbox_MACBYTES);

  for (var i = 0; i < msg.length; i++) m[i] = msg[i];
...
nacl.secretbox.open = function(box, nonce, key) {
  ...
  checkLengths(key, nonce);
  var c = new Uint8Array(box.length);
  var m = new Uint8Array(box.length - 16);
  for (var i = 0; i < box.length; i++) c[i] = box[i];
...
nacl.secretbox.macLength = crypto_secretbox_MACBYTES;
...
nacl.box.macLength = crypto_box_MACBYTES;
